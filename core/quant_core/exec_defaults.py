...
_INSTRUMENTS: dict[str, InstrumentSpec] = {
    ...
    # 미니는 정규와 호가단위가 다르다 — **0.02**(정규 0.05). 실측 2026-07-20(LS t8435 MF
    # 마스터: 미니 월물가가 전부 0.02 배수·0.05로는 비정수 / 1055.15 지정가 주문이
    # `01403 호가단위`로 특정 거부). 데이터는 정규 패널을 앨리어스로 쓰므로(PRICE_ALIAS)
    # 이 값이 미니의 유일한 고유 호가 정보다 — 백테스트 체결가 라운딩과 라이브 지정가
    # 라운딩(trader._round_limit) 양쪽이 소비한다(0.05로 반올림하면 브로커가 거부한다).
    "미니코스피200선물": InstrumentSpec("futures",  50_000.0, 0.02, "KRW", 0.198, 0.132, "kospi200_2nd_thu", "at_expiry"),
