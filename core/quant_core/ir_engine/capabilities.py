    ...
    return {
        ...
        "instruments": {
            ...
            "does": ("심볼 중 일부는 선물 상품 — 카탈로그가 승수·증거금·만기·통화를 안다(엔진 자동 인식, "
                     "IR에 자산클래스 표시 불필요). 선물 심볼: 원유선물·천연가스선물·금선물·"
                     "은선물(COMEX)·나스닥선물·비트코인선물. 주식처럼 universe.symbols에 이름만 넣는다. "
                     "(⚠ '코스피200선물'은 현재 ETF 프록시=주식 취급 — 실 선물 데이터 수급 전까지 승수 미적용.)"),
            "use_for": ("선물 = 내재 레버리지·숏 자연(direction=short, 차입 불필요). 백테스트는 추세추종"
                        "(always 보유마스크)·정기(scheduled)만 지원 — ⚠ 이벤트(on_signal) 단발 진입은 아직 "
                        "미지원(엔진 차단). 만기 롤·연속물·통화환산은 현재 미적용(단일 연속 시계열·단일통화 가정). "
                        "신호·take_profit/stop_loss(%)·hold_days는 주식과 동일(가격% 기준). "
                        "자본=증거금·손익=가격변화×승수×계약수(엔진 처리)."),
        },
