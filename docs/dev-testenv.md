...
- **도구시도 자기교정(`--max-turns 3`).** 모델이 `_NOTOOL_ADDENDUM`에도 native tool_use를
  시도하는 샘플이 있다(동일 쿼리 실측 num_turns=2: 1턴째 도구시도·거부→2턴째 JSON 자기교정).
  max-turns 1이면 그 샘플은 최종텍스트 없이 끝나 빈 result → 재시도가 같은 샘플이면 "3회 모두
  실패"(복잡 쿼리일수록 확률↑). 3으로 거부 피드백 후 자기교정할 여유를 준다.
