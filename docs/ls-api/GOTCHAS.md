...
### G-DF3 해소 🔴 **`t0434` status 어휘 확정 = {접수, 취소확인, 완료}** — "완료"는 체결이 아니다
- 종전 `ls_futures_broker.order_status`에 `⚠ G-DF3: status 문자열 실측 전` 주석이 있었다. 실측 전수 교차표:

  | status | cheqty | ordrem | orgordno | 실제 의미 | 건수 |
  |---|---|---|---|---|---|
  | 접수 | 0 | >0 | 0 | 미체결 살아있음 | 20 |
  | 완료 | >0 | 0 | 0 | **체결 완료** | 19 |
  | **완료** | **0** | **0** | **0** | **취소로 종결(체결 없음)** | **24** |
  | 취소확인 | 0 | 0 | ≠0 | 취소 확인행(자식) | 27 |

- 🔴 **함정**: `완료`가 체결과 취소 **양쪽**에 쓰인다. 문자열만 보면 구분 불가 — `cheqty`로 갈라야 한다.
  종전 코드는 `"취소" in status` → `cheqty>0` 순으로만 봐서 **`완료/che=0`이 어느 분기에도 안 걸리고
  마지막 `else: submitted`(살아있음)로 떨어졌다** ⇒ 취소된 주문이 당일 내내 pending에 잔존 →
  로컬앱 "미체결 0" 게이트 차단 → **무인 자동 업데이트까지 차단**(mwmw 실계좌 좀비의 당일 메커니즘).
- **판정은 문자열이 아니라 수량으로**: `ordrem>0` 이면 살아있음, `ordrem==0 && cheqty==0` 이면 체결
  없이 종결(cancelled). 어휘가 추가돼도 안 깨진다.
- 취소 확인행은 **자식 주문번호**로 온다(`ordno 54 · orgordno 29`) — 원주문번호로 조회하면 안 잡힌다.
  `pending_orders`는 이미 `orgordno≠0` 행을 제외해 정상.
