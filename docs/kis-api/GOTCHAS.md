...
## 2026-07-20 국내선물 모의 실측 (japan1 · 하니스 `퀀트/measure/mrun.py` kis_mid)

원시 캡처 = `퀀트/measure/out/2026-07-20/raw.jsonl`. 통제 시퀀스 전 단계 성공.

### K-ORD1 🟢 **`TTTO5105R` 주문가능수량 = 신규 전용**(청산분 미포함) — 게이트 해제
- 실측: **무보유** 상태 buy 9 / sell 9 → **롱 1 보유** 시 sell **8** / buy **8**.
- 합산(신규+청산) 의미라면 롱1 보유 시 sell은 9(신규8+청산1) 이상이어야 하는데 **8로 감소**
  ⇒ 청산 여력이 안 섞인 **신규 전용** 확정. LS `NewOrdAbleQty`와 같은 의미.
- ⇒ `KisFuturesBroker.ORDERABLE_NEW_ONLY = True` 전환 가능(§18.2 방향무관 리버설 크레딧을
  KIS에도 적용). 종전엔 미실측이라 같은-편 크레딧으로 안전 강등돼 있었다.

### K-ORD2 부분취소·감량정정 **둘 다 수리** 🟢 — 단 `RMN_QTY_YN`이 실제 수량을 지배한다 🔴
- 부분취소(`RVSE_CNCL_DVSN_CD=02`·`RMN_QTY_YN=N`·수량 1 of 2): `40630000` 성공.
- 감량정정(`01`): `40620000` 성공. 두-주문(연속 매도 2건)도 각각 `40590000` 수리.
- 🔴 **`RMN_QTY_YN`은 문서대로 동작한다 — `Y`면 `ORD_QTY`를 무시하고 잔량 전부를 취소한다.**
  대조 실측(같은 날 `kis_partial` 스텝, 매수 3계약 주문):

  | RMN_QTY_YN | ORD_QTY | 잔량 |
  |---|---|---|
  | (주문 직후) | — | 3 |
  | **N** | 1 | **2** — 정확히 1만 취소 |
  | **Y** | 1 | **0** — 남은 **2 전부** 취소 |

  두 취소 모두 `40630000`(성공)을 돌려준다 ⇒ **응답만으로는 구분 불가**. 반드시 취소 후
  미체결 잔량(`qty` 필드 — `ord_qty`는 원 주문수량이라 안 줄어든다)을 재조회해 확인할 것.
- ⚠ **함정**: `KisFuturesBroker.build_futures_cancel_body`는 `RMN_QTY_YN="Y"`를,
  `KisBroker.cancel`은 `QTY_ALL_ORD_YN="Y"`를 **하드코딩**한다. 이 상태로 "1 of 3 부분취소"를
  요청하면 3 전부 취소된다(동시호가 가드가 유저 수동 주문을 통째로 날리는 부류).
  부분취소 의사를 어댑터까지 명시 전달해야 한다.
- 💡 교훈: 하니스가 플래그를 덮어쓴 채 측정하면 **어댑터 경로를 검증한 게 아니다**.
  오전 `kis_mid`는 `krvse(..., rmn="N", ...)`로 우회 호출해 "부분취소 수리"만 확인했고,
  프로덕션 코드의 `Y`는 손대지 않은 채였다.

### K-CCL1 체결내역(`inquire-ccnl`) 행 필드 — 주문번호 **공백 패딩** 주의
- `odno: "      5971"`, `orgn_odno: "         0"` — **좌측 공백 패딩**. 비교 전 `strip()` 필수
  (가드 `_canon`은 strip + `lstrip("0")`이라 정합).
- `pdno: "A01609"` — **6자**(LS는 8자 `A0169000`). 같은 계약도 브로커마다 코드 폭이 다르다.
- 기타: `sll_buy_dvsn_cd`(02=매수) · `ord_idx`(주문가) · `avg_idx`(체결평균가) · `tot_ccld_qty` ·
  `rjct_qty` · `ord_tmd`(HHMMSS) · `ingr_trad_rjct_rson_cd/name`(거부사유).
- 🔴 `cano`·`acnt_prdt_cd`(계좌번호)가 응답에 포함 — 캡처·로깅 시 마스킹 필수(LS G-SEC1과 동일 부류).

### K-PRC1 `FHKST01010200`(주식 예상체결가) 장외에도 응답
- 일요일 조회에 금요 종가 반환, 장중 조회에 현재가(005930 → 241,000) 반환 — 폴백 정상.
