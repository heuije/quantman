...
- **원인**: KRX 파생 **실시간가격제한** — 직전약정가 ±1%(코스피200선물)를 넘는
  ...
  틱 *올림*하면 정확히 1틱 초과.
- **해결**: `trader._live_limit`이 KRX 선물 지정가를 현재가×(1±1%) **밴드 안쪽
  틱 라운딩**(매수 내림·매도 올림)으로 클램프 (`_KRX_FUT_RT_BAND_PCT`).
- **우리 코드**: `local/localapp/trader.py` `_live_limit` ·
  회귀 `local/tests/scenarios/test_order_price_plane.py`.
- ⚠ 타 상품(코스닥150선물 등) 밴드폭은 상품별 상이 — 라이브 확장 시 확인.
