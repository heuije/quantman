...
- `local/localapp/kis_broker.py::expected_fill_price` — §18 주식 크레딧 기준가(2026-07-19):
  `output2.antc_cnpr`(동시호가 예상체결가) > 0이면 그 값, 아니면 `output2.stck_prpr`(현재가).
  소비자 = `Trader._stock_credit_price`(청산·되돌림 회수 현금 추정 — 실패 시 현재가→번들 종가 폴백).
