...
## ✅ 프로덕션 실측 (2026-07-18 · Railway · 회사 실전키 QP_KIS_DATA_*)

- 첫 수집 성공: **348일 커버(2026-05-19~2027-05-01)** = 오늘−60일 백필 + 미래 약 10.5개월
  → 과거 `BASS_DT` 조회 허용·연속조회 페이지네이션 동작 확인.
- **이중신호 실증**: 2026-07-17(임시휴장)을 라이브러리=개장 vs KIS=휴장으로 감지 →
  KIS 적용(`kis_overrides: 1`) — 로드맵 A의 표적 사례가 첫 부팅에서 교정됨.
- 미확인 잔여: 페이지당 정확 행 수(총량만 확인) · 모의투자 도메인 지원 여부(실전키 사용 중이라 무관).
