...
- **"실측 대기"로 보류하기 전에 API 문서의 *필드 이름*부터 읽어라 — 이미 답이 있을 수 있다.**
  §18 빈-상태 사이징은 "`orderable_qty(sell)`이 청산분을 포함하는지 미검증"을 이유로 반대방향
  크레딧을 3주간 보류했다. 실제로는 LS `CFOAQ10100`이 `OrdAbleQty`(총)·`NewOrdAbleQty`(신규)·
  `LqdtOrdAbleQty`(청산)를 **분리 반환**(예시 `38 = 36 + 2`)하고 우리는 신규만 읽고 있었다 —
  **필드명 자체가 "신규 전용"이라 답이었고, 문서 1줄이면 끝날 일이었다.** 그 사이 실유저(mwmw)는
  반대편 수동 보유가 있을 때마다 **정확히 절반 언더사이징**으로 매매했다(2026-07-16 실측: 롱4가
  나와야 하는데 롱2). 교훈 = *미검증 가정을 게이트로 걸 때, 그 가정이 문서/필드명으로 즉시 확정
  가능한지 먼저 확인*. 브로커 여력 API는 대개 **신규/청산/총**을 나눠 준다고 의심하라. 〔작성: 조대표〕
- **넷팅(물리 발주 부호)과 사이징(목표 크기)은 다른 질문 — side 규칙을 같이 쓰지 마라.**
  §19 A1은 자전거래 회피를 위해 pending을 **같은 방향만** 넷팅하는데, 이 "같은 방향만"을 사이징
  크레딧에도 그대로 적용해 반대편 수동 보유가 크레딧을 못 받았다. 넷팅은 *부호가 뒤집히면 안 되는*
  물리 제약이고, 사이징은 *"수동을 취소하면 얼마까지 가능한가"* 라는 가정적 질문이다 —
  후자는 방향과 무관하다. 크레딧은 **소스별로** 규칙이 갈린다: 수동/외부 보유=방향 무관(원복 전제),
  전략 청산=같은 편만(리버설은 남은 여력 기준이 유저 모델). 〔작성: 조대표〕
