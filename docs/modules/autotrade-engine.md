...
- **가격 평면 정책 — 신호·사이징 근거는 데이터 평면, 주문 가격·밴드는 거래 평면(발주 직전
  ...
  failed로 마감해 당일 재시도 허용(D5-5). **KRX 선물은 정적 상·하한 외에 실시간가격제한
  (직전가 ±1%, 코스피200선물)이 binding** — tolerance 1%의 틱 올림이 밴드를 1틱 초과해 항상
  거부됐다(06-11 실측 2건). `_live_limit`이 밴드 안쪽 틱 라운딩(매수 내림·매도 올림,
  `_KRX_FUT_RT_BAND_PCT`)으로 클램프. 〔작성: 조대표〕
