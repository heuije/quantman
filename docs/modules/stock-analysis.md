...
- **기간 수익률(1/3/6/12개월)은 달력(캘린더) 앵커 — 고정 거래일수 금지.** describe `_ret`가 종전 21/63/126/252 **거래일**을 썼는데, 한국장은 연 ~246거래일이라 **252거래일이 실제로는 ~12.4개월(377일) 전**을 가리켜 "12개월"이 달력 1년과 어긋났다(희제 실사용 신고: 코스닥 12m가 네이버·타경로와 불일치 −2.68% vs −4.78%). → `asof - pd.DateOffset(months=N)` 이하 최근 종가 대비로 계산해 "12개월"=진짜 1년으로 맞춘다. **⚠ 구분: `pct_change_Nd`(1d/5d/20d/252d) 신호 지표는 이름 그대로 N'일' 모멘텀이 정의라 거래일 유지**(스크리닝·이벤트·백테스트 신호 — 사용자에게 보여주는 "N개월 수익률"이 아님). **52주 고저는 ~252거래일 유지**(엑셀 원자료 라이브 수식과 정합·range 지표라 2주 앵커차 둔감). 엑셀 요약은 describe `returns`를 그대로 읽어 자동 반영. 회귀=`test_question_describe_p2.py::test_single_report_golden`.
