...
- 진행: **P0(#253 `51eb5aa`)·P1(#258 `3f49e97`) 머지·배포·prod LIVE.** **P2-A(KR 코어패널 2010 깊이) 구현·검증·push·draft [PR#264](https://github.com/MercKR/quantman/pull/264)**(머지 허락대기): OHLCV 신규 깊이백필+cron / flow·컨센 floor 2010 / 재무 2015. **P2-B 재설계(공식 KRX Open API 채택)**: OI+옵션P/C+ETF flow+V-KOSPI+채권/국채를 **공식 KRX API**(`data-dbg.krx.co.kr`·AUTH_KEY·2010~)로 — LS는 *선물 투자자 수급(t8462)*에만(공식 API 투자자분해 없음). 단계 S1(시장지표)→S2(선물OI)→S3(ETF flow)→S4(KOSPI200 OHLCV 전환·선물세션 조율)→S5(LS 수급). **S1 완료·라이브검증·커밋 feb3f85**(branch `feat/data-krx-openapi`·미push): `feeds/krx_openapi.py` 시장지표 5종 매크로형 시계열. **P2-C(Tier1)→공식 KRX API로 무료 재편입**(보류 해제). 미국채=FRED 기존(추가불요). 교훈은 §교훈 distill.
