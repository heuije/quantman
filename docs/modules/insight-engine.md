...
- **3a(교차캘린더) 보류**: 정렬은 `ir_engine/`+`market_calendar.py`+`calendars/`(엔진 도메인)이나, 데이터엔진 세션의 캘린더-인접 작업(#267 S4 'KOSPI200 OHLCV·선물세션 조율')과 겹칠 위험 → **데이터 세션과 조율 후 진행**(CLAUDE.md 충돌 회피·사장님 "데이터 세션 참고" 지시).
- 교훈: **0거래는 원인(신호없음 vs 자본부족)을 구분해야 정직하다 — 엔진의 침묵 스킵을 카운트해 표면화**(단방향 낙관 파이프 패턴). 증거금률 인상(PR#266)이 자본부족 빈도를 키우므로 사이징 변경과 표면화는 짝.
