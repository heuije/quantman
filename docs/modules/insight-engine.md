...
- **⚠ 외부 "SYM.x" 참조의 가용성 소유자는 R0(`available_refs`→validate)와 D-ref(무결성 게이트) 둘뿐 — 새 게이트를 더하지 말고 이 둘을 고칠 것.** 참조 심볼이 dataset에 아예 없으면(parquet 부재 → 로더가 키 생략) R0가 이미 정직 거부한다("데이터 없음"). 구멍은 **키는 있는데 df가 None/빈** 유입 — `available_refs`가 키를 무조건 유효 참조로 쳐 R0를 통과하고, 이벤트 패널이 NaN→False로 **success=True·0건 버킷**(조용한 오답)이 됐다(빈 df 키 제외로 마감·회귀=`core/tests/test_missing_ref_symbol_failloud.py`). 잔여 비대칭 2가지(둘 다 prod 검증경로 도달 불가·필요해지면 여기부터): ① 외부 ref는 심볼 존재만 검사 — 심볼은 있고 *컬럼*이 없으면 여전히 NaN→False(`__SELF__.x`는 컬럼 검사함; valid_refs가 flat set이라 per-symbol 컬럼 정밀검사 불가) ② `run_query` 직접 호출(검증 우회)은 데이터 가용성 무검사(계약 설계상 데이터 층은 R0/D-* 소유·prod 직접 호출자 없음). **"조용한 0건" 진단은 결과 dict의 `success`/`error`부터 볼 것** — buckets/status만 보면 정직 거부를 조용한 성공으로 오진한다.
- **⚠ universe.screener의 소비자는 choke point(`_universe_symbols`) 하나 — 러너의 사적 유니버스 축소 금지.** 스터디 러너 ~19종이 screener를 조용히 무시해 "코스닥만" 이벤트스터디가 전체(미국 포함) 유니버스로 돌고 자기서술은 '2차 스크리너: 적용'으로 거짓 광고했다(prod 실측 — **필터 전/후 완전 동일 수치가 결정적 신호**·구성에 US 티커). 컴파일러는 무죄(IR에 스크리너 정확 존재)·#354는 emit과 백테스트/select 실행만 검증해 스터디 소비 계층이 사각. 이제 `_universe_symbols`가 `_screener_mask` 최신 유효 단면 1회 평가로 소비(속성 조건=날짜 무관 정확·가격 조건='최신 단면 자격' 정규화)하고, `run_select`만 자체 PIT as-of 마스크 소유라 `apply_screener=False` opt-out. **새 러너가 유니버스를 만들 땐 반드시 `_universe_symbols` 경유** — "재시도해도 동일 수치" 신고는 필터 미소비부터 의심. 회귀=`core/tests/test_study_screener_universe.py`.
- **⚠ 챗의 분석형 질의(이벤트 스터디 등)를 `simulate(nl)`로 표현하면 compile_nl이 팬아웃·병목의 근본 — 결정적 조립 도구로 우회.** chat-perf 실측(급등 전조 질의, 로컬 재현): 오케스트레이터가 "공통 전조증상"류를 후보 지표마다 별도 `simulate`로 쪼갬 → 각 `compile_strategy→compile_nl`이 건당 **48~147초**(수리 루프 2회=Sonnet 3콜)이고 이벤트 스터디 IR이 스키마 검증(`signal` 필수 등) 실패로 재시도까지 → 8+콜×~75초 = **10분+**. **데이터/프리필터는 무죄**(코스피 945종목 warm 1~5초·`_universe_symbols` 정상). 근본해법 = `analyze_event` 도구: `screen`/`compare`처럼 `assemble_ir` 방식으로 `relate`+event IR을 **코드로 결정적 조립**(compile_nl 0), 후보 지표 배열을 1콜로 순차 실행(warm 캐시 재사용)·`market`→Market 스크리너(프리필터)·음수창=전조/양수창=forward·`hit_threshold`=역확률 hit-rate. 실측: 지표당 ~10초, 라우팅 첫 라운드 8초에 analyze_event 1콜로 6지표 묶음 emit. 교훈: **챗 분석형은 느린·비결정 NL 컴파일을 핫패스에서 제거 — 구조화 입력→결정적 IR 조립 도구가 정답**([[arch_four_layer_contract]] 챗판: 도구스키마·run·프롬프트 라우팅·compact 렌더 4계층 동시 배선). ⚠ 이벤트 스터디는 forward/전조 **가격수익 경로**만 측정(`target_node` 미소비 — LLM이 target_node=지표로 시도한 게 애초 무효)·지표는 event 조건으로. 회귀=`server/tests/test_chat_tools.py::test_build_event_ir_*`.
...
### [2026-07-12] 스터디 러너 universe.screener 조용한 무시 — choke point 소비로 부류 마감 [완료·draft PR·머지 대기]
- 의도: 프로덕션 실측(로컬 $0 env conv=5 "코스닥 급등 전조")에서 Market=KOSDAQ 스크리너가 IR에 정확히 컴파일됐는데도 이벤트스터디가 전체(미국 포함) 유니버스로 실행돼 필터 전/후 완전 동일 결과(34,720건·US 티커 구성)를 재생산하고 자기서술은 '적용'으로 표시하던 조용한 오답 부류의 근본 수정. 원인=screener 소비가 러너별 사적 구현(engine 백테스트·run_select만)·스터디 공용 `_universe_symbols`(17개 호출부)는 kind만 읽고 미소비(구주석 "직교 적용"은 의도 서술).
- 구현: `_universe_symbols`가 screener.condition을 `_screener_mask` 최신 유효 단면 1회 평가로 소비(속성 조건 정확·가격 조건 '최신 단면 자격' 정규화 주석 명시) + `run_select` opt-out(자체 PIT as-of 보존) + explain.py 이벤트 요약 stale 문구("발생 이후만·전조 아님" — WS1 음수 창 1급 이후 거짓·"+-252" 표기 오류 포함) 창 부호 기반 서술로 정정(계약 does와 정합).
- 검증: 부류 테스트 4종 red→green(`test_study_screener_universe.py` — choke 단위·opt-out·E2E 구성⊆KOSDAQ) + **실패한 실제 IR을 실데이터 리플레이**(KOSDAQ 1,726+US 256·실 ticker_db): 필터 O=10,788건·비-KOSDAQ 0건 / 필터 X=10,917건·US 174건(대조) + 전 스위트 green.
- 교훈: 위 §교훈 "universe.screener 소비자는 choke point 하나" 항목으로 distill.
