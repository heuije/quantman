"""KRX 선물(K200·KQ150) 데이터 갱신 — 만기물 패널 경로 배선 검증(S4).

_refresh_krx_futures가 KRX_API_KEY 미설정이면 no-op, 설정이면 fetch_futures_panel로
최근 윈도우를 수집(등록 상품 전체·하루 1콜)·캐시 무효화하는지. 패널→연속물 stitch 로직
자체는 core test_krx_openapi(fetch_futures_panel_saves_and_rebuilds)·test_futures_roll이 커버.
