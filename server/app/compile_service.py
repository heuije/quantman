...
_MACRO_ALIASES: dict[str, str] = {
    ...
    # 환율·달러 — 통용 티커/영문을 정식 FRED 수집명으로(원달러환율=DEXKOUS·무역가중달러지수=DTWEXBGS).
    # 프로덕션 실측(#D): 봇이 inspect(USDKRW/USD/KRW/KRW=X)로 4연속 조회 실패했으나 시계열은
    # '원달러환율'로 이미 수집돼 있었다 — 데이터 갭이 아니라 심볼 해상도 갭이었다.
    "usdkrw": "원달러환율", "usd/krw": "원달러환율", "krw=x": "원달러환율", "krwusd": "원달러환율",
    "원달러": "원달러환율", "원/달러": "원달러환율", "달러원": "원달러환율", "달러환율": "원달러환율",
    "환율": "원달러환율",
    "dxy": "무역가중달러지수", "달러인덱스": "무역가중달러지수", "달러지수": "무역가중달러지수",
    "dollar index": "무역가중달러지수",
    # KR 선물·ETF 수급(flow_deriv_kr 6심볼) — 통용 축약(코스피선물↔코스피200선물)·소문자
    # (lookup이 .lower()라 KRETF 원명도 별칭 필요)·무수식 "선물"=대표 코스피200(관용) 해상도.
    "코스피선물외국인순매수": "코스피200선물외국인순매수",
    "코스피선물기관순매수": "코스피200선물기관순매수",
    "코스닥선물외국인순매수": "코스닥150선물외국인순매수",
    "코스닥선물기관순매수": "코스닥150선물기관순매수",
    "선물외국인순매수": "코스피200선물외국인순매수",
    "선물기관순매수": "코스피200선물기관순매수",
    "kretf외국인순매수": "KRETF외국인순매수", "kretf기관순매수": "KRETF기관순매수",
    "etf외국인순매수": "KRETF외국인순매수", "etf기관순매수": "KRETF기관순매수",
}


def resolve_symbol_alias(symbol: str) -> str:
    """통용 티커·이름(USDKRW·DXY 등) → 정식 데이터 심볼키. 미매칭은 원문 유지.

    compile 경로는 name_map(=이 별칭 병합)로, inspect 경로(chat.tools.run_inspect)는 이 함수로 —
    단일 별칭 소스를 공유해 '컴파일은 되는데 inspect는 안 되는' 해상도 분기를 없앤다(#D)."""
    return _MACRO_ALIASES.get((symbol or "").strip().lower(), symbol)
