...
- **거래대금·공매도·선물 포지셔닝(신규 필드 의미)**:
  ...
  · **KR 선물·ETF 투자자 수급**도 심볼로 참조한다: `코스피200선물외국인순매수`·`코스피200선물기관순매수`·`코스닥150선물외국인순매수`·`코스닥150선물기관순매수`·`KRETF외국인순매수`·`KRETF기관순매수`(일별 순매수 **대금·원**, 부호 있음 — 양수=순매수·음수=순매도, 2010~). "외국인이 선물을 파는 날" = `코스피200선물외국인순매수.Close < 0`, "N일 연속 순매도"는 그 조건을 modifier(지속 N일)로, "N일 누적"은 `ts_sum(window=N)` 조합. ⚠ 종목별 수급(`inst_net_buy`·`foreign_net_buy`)과 별개 — 이건 선물 상품/ETF 시장 전체 단위다.
...
14b. [전략 합성(병행 포트폴리오·WS3)] "전략 A와 B를 *병행/합산*하면"·"이와 별개로 ~도 병행합니다"·"둘을 합친
    포트폴리오"처럼 *여러 완결 전략의 혼합 성과*가 답이면 → query="simulate" + **components**=[{{"weight": 1,
    "ir": <완결 전략 IR dict>}}, ...] (2개+·weight=상대 배분비·top-level signal은 명목 data Close·study 없음).
...
14c. [적립식(DCA)·주기 납입(WS4)] "매달 100만원씩 매수/적립"·"월 N원씩 투자하면"처럼 *주기적 신규 자본 유입*이
    ...
    **시간가중(TWR·납입 왜곡 제거)**이고 원금대비 수익은 contributions에 별도 동봉 — fixed_amount 사이징으로
    흉내내지 말 것(그건 기존 자본 내 예산). ⚠ vol_target·max_drawdown_stop 오버레이와 동시 사용 불가(정직 거부).
...
15. [이벤트 스터디 — 신호 후 수익·방향성·단일종목 예측] "롱/숏 신호(돌파·골든크로스 등) 발생 후 N일 수익이
    ...
    **여러 종목에 각각 검증(코호트 비교)**: "이 조건이 삼성·에코·현대차 각각에 통하나"·"여러 종목에 같은
    신호 검증"처럼 *동일 이벤트스터디를 여러 종목에 나란히 비교*면 → universe.kind="list"(종목들) +
    study.axis="entity" + study.assets=[종목코드들] (+ study.event·windows는 그대로). 엔진이 종목별로 각각
    실행해 per-symbol n_events·유의성을 한 표로 낸다(list 유니버스 단독의 pool 통계와 구분 — 개별 비교).
    ⚠ 종목마다 relate를 따로 호출하지 말고 **한 번에 코호트로 펼친다**(종목당 반복 호출은 지연·컴파일 실패 폭발).
